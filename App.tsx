...
// Route-level components (each pulls in maps/animation code) load on demand;
// the first paint only needs the shell, header, and home content.
const BusinessProfile = React.lazy(() => import('./components/BusinessProfile'));
const ServiceProfile = React.lazy(() => import('./components/ServiceProfile'));
const NeighborhoodPage = React.lazy(() => import('./components/NeighborhoodPage'));
const BusinessDirectory = React.lazy(() => import('./components/BusinessDirectory'));
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (view === 'blog' && selectedBlog) {
      const content = getGuidePageContent(selectedBlog, COST_GUIDES, CATEGORY_GROUPS);
      title = content.title;
      ...
      const content = getGuidesHubContent(COST_GUIDES);
      title = content.title;
      description = content.description;
      canonicalPath = content.path;
      robots = content.robots;
  ...
  // Real per-page numbers + the matching cost guide(s), computed by the same
  // functions the prerender uses - so the hydrated page keeps the substance
  // (and the internal links) the static HTML shipped with.
  const categoryPageExtras = useMemo(() => {
    if (!selectedCategoryPageCategory) return null;
    const cityScope = view === 'categoryCityPage' ? selectedCategoryPageCity : null;
    const content = getCategoryPageContent(selectedCategoryPageCategory, cityScope, services);
    return {
      insightsText: describeInsights(content.insights, selectedCategoryPageCategory, cityScope),
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans antialiased">
            ...
            (() => {
              ...
              return (
                <section className="pt-12">
                  ...
                  {categoryPageExtras?.insightsText && (
                    <p className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-700 max-w-3xl mb-4">{categoryPageExtras.insightsText}</p>
                  )}
                  {categoryPageExtras && categoryPageExtras.guides.length > 0 && (
                    <p className="text-gray-700 max-w-3xl mb-8">
                      What does it cost? Read the full guide:{' '}
                      {categoryPageExtras.guides.map((g: any, i: number) => (
                        <React.Fragment key={g.slug}>
                          {i > 0 && ' · '}
                          <Link href={guidePath(g.slug)} onNavigate={() => handleBlogClick(g)} className="text-primary font-semibold hover:underline">{g.title}</Link>
                        </React.Fragment>
                      ))}
                    </p>
                  )}
              ...
              <div className="text-gray-700">
                ...
                {(selectedBlog as any).category && (
                  <div className="mt-10 pt-8 border-t border-gray-100">
                    <Link
                      href={categoryPath((selectedBlog as any).category)}
                      onNavigate={() => handleCategoryPageClick((selectedBlog as any).category)}
                ...
                {(() => {
                  const related = getGuidePageContent(selectedBlog as any, COST_GUIDES, CATEGORY_GROUPS).relatedGuides;
                  if (!related.length) return null;
                  return (
                    <div className="mt-12 pt-8 border-t border-gray-100">
                      <h2 className="text-xl font-extrabold text-gray-900 mb-3">More cost guides</h2>
                      ...
                      <Link href={GUIDES_HUB_PATH} onNavigate={() => setView('articles')} className="inline-block mt-4 text-primary font-bold hover:underline">See all cost guides &rarr;</Link>
                    </div>
                  );
                })()}
              </div>
      ...
      <Footer onNavigate={(page) => setView(page as any)} onCategoryClick={(category) => handleCategoryPageClick(category)} onGuideClick={(guide) => handleBlogClick(guide)} />
